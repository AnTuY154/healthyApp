...
export function DiaryCard ({ description, time }: DiaryCardType): JSX.Element {
  ...
  useEffect(() => {
    const width = containerRef?.current?.getBoundingClientRect().width ? containerRef.current.getBoundingClientRect().width : 0
    if (width !== 0) {
      setHeight(width)
  ...
  return (
    <div ref={containerRef} style={{ height }} className="diary_card">
